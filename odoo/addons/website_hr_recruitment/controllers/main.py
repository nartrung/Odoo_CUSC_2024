...
from odoo.http import NotFound
...
class WebsiteHrRecruitment(http.Controller):
    ...
    @http.route('/job/applied', type='http', auth="user", website=True)
    def job_partner(self, **kwargs):
        # Lấy partner_id của người dùng đăng nhập
        current_user_partner_id = request.env.user.partner_id.id

        # Truy vấn dữ liệu từ model hr.applicant
        applicants = request.env['hr.applicant'].sudo().search([('partner_id', '=', current_user_partner_id)])

        # Truy vấn danh sách các job từ model hr.job
        jobs = request.env['hr.job'].sudo().search([])

        # Truy vấn danh sách các department từ model hr.department
        departments = request.env['hr.job'].sudo().search([])

        # Tạo một dictionary để lưu trữ job_id và tên của job tương ứng
        job_name_dict = {}
        for job in jobs:
            job_name_dict[job.id] = job.name

        # Tạo một dictionary để lưu trữ department_id và tên của job tương ứng
        department_name_dict = {}
        for department in departments:
            department_name_dict[department.id] = department.name

        return request.render("website_hr_recruitment.applicant_template", {
            'partner_id': current_user_partner_id,
            'applicants': applicants,
            'job_name_dict': job_name_dict,
            'department_name_dict': department_name_dict,
        })



    # @http.route('/job/applied/detail/<int:id>', type='http', auth="public", website=True)
    # def job_partner_detail(self, id, **kwargs):
    #     # Truy vấn danh sách các job từ model hr.job
    #     jobs = request.env['hr.job'].sudo().search([])
    #
    #     # Truy vấn danh sách các department từ model hr.department
    ...
    #     for department in departments:
    #         department_name_dict[department.id] = department.name
    #     applicant = request.env['hr.applicant'].sudo().search([('id', '=', id)])
    #     return request.render("website_hr_recruitment.applicant_detail", {
    #         'applicant': applicant,
    #         'job_name_dict': job_name_dict,
    #         'department_name_dict': department_name_dict
    #     })

    @http.route('/job/applied/detail/<int:id>', type='http', auth="user", website=True)
    def job_partner_detail(self, id, **kwargs):
        # Truy vấn danh sách các job từ model hr.job
        jobs = request.env['hr.job'].sudo().search([])
        current_user_partner_id = request.env.user.partner_id.id
        applicant = request.env['hr.applicant'].sudo().search([('id', '=', id)])
        if applicant and applicant.partner_id.id != current_user_partner_id:
            # Nếu không tìm thấy đơn ứng viên hoặc người dùng không có quyền truy cập đơn này, thực hiện hành động phù hợp.
            raise NotFound()
        if not applicant:
            # Nếu không tìm thấy đơn ứng viên hoặc người dùng không có quyền truy cập đơn này, thực hiện hành động phù hợp.
            raise NotFound()
